...
binary_survival_para_hazard_ipw = function(dat, time.list, misspecify = NULL, censor = "Cox"){
  ...
  G_Z1_A1 = G_Z1_A0 = G_Z0_A1 = G_Z0_A0 = matrix(1, length(time.list), nrow(dat))
  omega_Z1_A1 = omega_Z1_A0 = omega_Z0_A1 = omega_Z0_A0 = matrix(0, length(time.list), nrow(dat))
  
  ## when t=1 
  phi1_1 = mean(((dat$obs.Y >= time.list[1])*dat$Z*dat$A/(G_Z1_A1[1,]*pi_1*delta_1)))*
    mean((dat$A*dat$Z / (delta_1))) + 
    mean(((dat$obs.Y >= time.list[1])*dat$Z*(1-dat$A)/(G_Z1_A0[1,]*(1-pi_1)*delta_1)))*
    mean(((1-dat$A)*dat$Z / (delta_1)))
  
  phi1_0 =  mean(((dat$obs.Y >= time.list[1])*(1-dat$Z)*dat$A/(G_Z0_A1[1,]*pi_0*delta_0)))*
    mean((dat$A*(1-dat$Z)/ (delta_0)) ) + 
    mean(((dat$obs.Y >= time.list[1])*(1-dat$Z)*(1-dat$A)/(G_Z0_A0[1,]*(1-pi_0)*delta_0)))*
  ...
  ifvals[, 1] = mean(phi1_1 - phi1_0) / mean(phi2_1- phi2_0) 
  
  if("censoring" %in% misspecify){
    newdata_Z1_A1_censor = data.frame(Z = rep(1, nrow(dat)), W.1 = dat$W.1, 
                                      W.2 = dat$W.2, W.3 = dat$W.3, W.4 = dat$W.4, W.5 = dat$W.5,
                                      A = rep(1, nrow(dat)))
    newdata_Z1_A0_censor = data.frame(Z = rep(1, nrow(dat)), W.1 = dat$W.1, 
                                      W.2 = dat$W.2, W.3 = dat$W.3, W.4 = dat$W.4, W.5 = dat$W.5,
                                      A = rep(0, nrow(dat)))
    newdata_Z0_A1_censor = data.frame(Z = rep(0, nrow(dat)), W.1 = dat$W.1, 
                                      W.2 = dat$W.2, W.3 = dat$W.3, W.4 = dat$W.4, W.5 = dat$W.5,
                                      A = rep(1, nrow(dat)))
    newdata_Z0_A0_censor = data.frame(Z = rep(0, nrow(dat)), W.1 = dat$W.1, 
                                      W.2 = dat$W.2, W.3 = dat$W.3, W.4 = dat$W.4, W.5 = dat$W.5,
                                      A = rep(0, nrow(dat)))
    if(censor == "Cox"){
      coxfit.censor = coxph(Surv(obs.Y, R.censor) ~ Z + W.1 + W.2 + W.3 + W.4 + W.5 + A, data = dat)
    ...
    }else if(censor == "additive"){
      dat$obs.YY = dat$obs.Y + runif(dim(dat)[1], 0, 1)*10^(-3)
      design = cbind(dat$Z, dat$W.1, dat$W.2, dat$W.3, dat$W.4, dat$W.5, dat$A)
      colnames(design) = c("Z", "W.1", "W.2", "W.3", "W.4", "W.5", "A")
      addfit.censor = ahaz(Surv(dat$obs.YY, dat$R.censor), design)
      addbase.censor = predict(addfit.censor, type = "cumhaz")
      addbasehazard.censor = matrix(0, length(addbase.censor$times), 2)
      addbasehazard.censor[,2] = addbase.censor$times
      addbasehazard.censor[1,1] = addbase.censor$cumhaz[1]*(addbase.censor$times[1] - 0)
      for(t in 2:nrow(addbasehazard.censor )){
        addbasehazard.censor[t,1] = (addbase.censor$cumhaz[t] - addbase.censor$cumhaz[t-1])*(addbase.censor$times[t] - addbase.censor$times[t-1])
      }
    } 
  }else{
    newdata_Z1_A1_censor = data.frame(Z = rep(1, nrow(dat)), X.1 = dat$X.1, 
                                      X.2 = dat$X.2, X.3 = dat$X.3, X.4 = dat$X.4, X.5 = dat$X.5,
                                      A = rep(1, nrow(dat)))
    newdata_Z1_A0_censor = data.frame(Z = rep(1, nrow(dat)), X.1 = dat$X.1, 
                                      X.2 = dat$X.2, X.3 = dat$X.3, X.4 = dat$X.4, X.5 = dat$X.5,
                                      A = rep(0, nrow(dat)))
    newdata_Z0_A1_censor = data.frame(Z = rep(0, nrow(dat)), X.1 = dat$X.1, 
                                      X.2 = dat$X.2, X.3 = dat$X.3, X.4 = dat$X.4, X.5 = dat$X.5,
                                      A = rep(1, nrow(dat)))
    newdata_Z0_A0_censor = data.frame(Z = rep(0, nrow(dat)), X.1 = dat$X.1, 
                                      X.2 = dat$X.2, X.3 = dat$X.3, X.4 = dat$X.4, X.5 = dat$X.5,
                                      A = rep(0, nrow(dat)))
    
    if(censor == "Cox"){
      coxfit.censor = coxph(Surv(obs.Y, R.censor) ~ Z + X.1 + X.2 + X.3 + X.4 + X.5 + A, data = dat)
      basefit.censor = basehaz(coxfit.censor, centered = FALSE)
      hazardfit.censor = matrix(0, nrow(basefit.censor), 2)
      hazardfit.censor[,2] = basefit.censor$time
      hazardfit.censor[1,1] = basefit.censor$hazard[1]*(basefit.censor$time[1] - 0)
      ...
      addfit.censor = ahaz(Surv(dat$obs.YY, dat$R.censor), design)
      addbase = predict(addfit.censor, type = "cumhaz")
      ...
      addbasehazard.censor = matrix(0, nrow(tmp.cum), 2)
      addbasehazard.censor[,2] = as.integer(tmp.cum[,2])
      addbasehazard.censor[1,1] = tmp.cum[1,1]*(tmp.cum[1,2] - 0)
      for(t in 2:nrow(tmp.cum)){
        addbasehazard.censor[t,1] = (tmp.cum[t,1]-tmp.cum[t-1,1])*(tmp.cum[t,2] - tmp.cum[t-1,2])
      }
    } 
  }  
  ...
  for(j in 2:k){
    ...
    if(censor == "Cox"){
      ...
      omega_Z1_A1[j,] = 1 - exp(-(baselines.h1)*exp(as.numeric(as.matrix(newdata_Z1_A1_censor) %*% as.matrix(summary(coxfit.censor)$coefficients[1:7,1])))) /
        exp(-(baselines.h0)*exp(as.numeric(as.matrix(newdata_Z1_A1_censor) %*% as.matrix(summary(coxfit.censor)$coefficients[1:7,1])))) 
      omega_Z1_A0[j,] = 1 - exp(-(baselines.h1)*exp(as.numeric(as.matrix(newdata_Z1_A0_censor) %*% as.matrix(summary(coxfit.censor)$coefficients[1:7,1])))) /
        exp(-(baselines.h0)*exp(as.numeric(as.matrix(newdata_Z1_A0_censor) %*% as.matrix(summary(coxfit.censor)$coefficients[1:7,1])))) 
      
      omega_Z0_A1[j,] = 1 - exp(-(baselines.h1)*exp(as.numeric(as.matrix(newdata_Z0_A1_censor) %*% as.matrix(summary(coxfit.censor)$coefficients[1:7,1])))) /
        exp(-(baselines.h0)*exp(as.numeric(as.matrix(newdata_Z0_A1_censor) %*% as.matrix(summary(coxfit.censor)$coefficients[1:7,1])))) 
      omega_Z0_A0[j,] = 1 - exp(-(baselines.h1)*exp(as.numeric(as.matrix(newdata_Z0_A0_censor) %*% as.matrix(summary(coxfit.censor)$coefficients[1:7,1])))) /
        exp(-(baselines.h0)*exp(as.numeric(as.matrix(newdata_Z0_A0_censor) %*% as.matrix(summary(coxfit.censor)$coefficients[1:7,1])))) 
      
    }else if(censor == "additive"){
      baselines.h = addbasehazard.censor[which.min(abs( addbasehazard.censor[,2] - time.list[j])),1] # baseline
      omega_Z1_A1[j,] = baselines.h + as.numeric(as.matrix(newdata_Z1_A1_censor) %*% as.matrix(summary(addfit.censor)$coefficients[1:7,1]))
      omega_Z1_A0[j,] = baselines.h + as.numeric(as.matrix(newdata_Z1_A0_censor) %*% as.matrix(summary(addfit.censor)$coefficients[1:7,1]))
      omega_Z0_A1[j,] = baselines.h + as.numeric(as.matrix(newdata_Z0_A1_censor) %*% as.matrix(summary(addfit.censor)$coefficients[1:7,1]))
      omega_Z0_A0[j,] = baselines.h + as.numeric(as.matrix(newdata_Z0_A0_censor) %*% as.matrix(summary(addfit.censor)$coefficients[1:7,1]))
    } 
    
    
    G_Z1_A1[j,] = G_Z1_A1[j-1,]*(1-omega_Z1_A1[j,])
    G_Z1_A0[j,] = G_Z1_A0[j-1,]*(1-omega_Z1_A0[j,])
    G_Z0_A1[j,] = G_Z0_A1[j-1,]*(1-omega_Z0_A1[j,])
    G_Z0_A0[j,] = G_Z0_A0[j-1,]*(1-omega_Z0_A0[j,])
    
    
    phi1_1 = mean(((dat$obs.Y >= time.list[j])*dat$Z*dat$A/(G_Z1_A1[j,]*pi_1*delta_1)))*
      mean((dat$A*dat$Z / (delta_1))) + 
      mean(((dat$obs.Y >= time.list[j])*dat$Z*(1-dat$A)/(G_Z1_A0[j,]*(1-pi_1)*delta_1)))*
      mean(((1-dat$A)*dat$Z / (delta_1)))
    
    phi1_0 =  mean(((dat$obs.Y >= time.list[j])*(1-dat$Z)*dat$A/(G_Z0_A1[j,]*pi_0*delta_0)))*
      mean((dat$A*(1-dat$Z)/ (delta_0)) ) + 
      mean(((dat$obs.Y >= time.list[j])*(1-dat$Z)*(1-dat$A)/(G_Z0_A0[j,]*(1-pi_0)*delta_0)))*
      mean(((1-dat$A)*(1-dat$Z) / (delta_0)))
...
binary_survival_para_hazard_plugin = function(dat, time.list, misspecify = NULL, survival = "Cox"){
  ...
  phi1_1 = 1*pi_1  + 1*(1-pi_1) 
  phi1_0 = 1*pi_0  + 1*(1-pi_0) 
...
binary_survival_para_if_hazard = function(dat, time.list,  misspecify = NULL, survival = "Cox", censor = "Cox"){
  ...
  if("censoring" %in% misspecify){
    ...
    if(censor == "Cox"){
      ...
      addbase.censor = predict(addfit.censor, type = "cumhaz")
      addbasehazard.censor = matrix(0, length(addbase.censor$times), 2)
      addbasehazard.censor[,2] = addbase$times
      addbasehazard.censor[1,1] = addbase$cumhaz[1]*(addbase.censor$times[1] - 0)
      for(t in 2:nrow(addbasehazard.censor )){
        addbasehazard.censor[t,1] = (addbase.censor$cumhaz[t] - addbase.censor$cumhaz[t-1])*(addbase.censor$times[t] - addbase.censor$times[t-1])
      }
    } 
    ...
    if(censor == "Cox"){
      ...
      tau = as.integer(max(dat$obs.Y)); tmp.cum = matrix(0, tau, 2)
      for(t in 1:tau){
        dummy = which(as.integer(addbase.censor$times) == t)[1]
        tmp.cum[t,] = c(addbase.censor$cumhaz[dummy], addbase.censor$times[dummy])
      }
      tmp.cum = na.omit(tmp.cum)
      addbasehazard.censor = matrix(0, nrow(tmp.cum), 2)
      addbasehazard.censor[,2] = as.integer(tmp.cum[,2])
...
binary_survival_para_naive = function(dat, time.list, misspecify = NULL, survival = "Cox", censor = "Cox"){
  ...
  ###### censoring function ########
  if("censoring" %in% misspecify){
    newdata_A1_censor = data.frame(W.1 = dat$W.1, 
                                   W.2 = dat$W.2, W.3 = dat$W.3, W.4 = dat$W.4, W.5 = dat$W.5,
                                   A = rep(1, nrow(dat)))
    newdata_A0_censor = data.frame(W.1 = dat$W.1, 
                                   W.2 = dat$W.2, W.3 = dat$W.3, W.4 = dat$W.4, W.5 = dat$W.5,
                                   A = rep(0, nrow(dat)))
    
    if(censor == "Cox"){
      coxfit.censor = coxph(Surv(obs.Y, R.censor) ~ W.1 + W.2 + W.3 + W.4 + W.5 + A, data = dat)
      basefit.censor = basehaz(coxfit.censor, centered = FALSE)
      hazardfit.censor = matrix(0, nrow(basefit), 2)
      hazardfit.censor[,2] = basefit.censor$time
      hazardfit.censor[1,1] = basefit.censor$hazard[1]*(basefit.censor$time[1] - 0)
      for(t in 2:nrow(hazardfit.censor)){
        hazardfit.censor[t,1] = (basefit.censor$hazard[t]-basefit.censor$hazard[t-1])*(basefit.censor$time[t] - basefit.censor$time[t-1])
      ...
      colnames(design) = c("W.1", "W.2", "W.3", "W.4", "W.5", "A")
      addfit.censor = ahaz(Surv(dat$obs.YY, dat$R.censor), design)
      addbase.censor = predict(addfit, type = "cumhaz")
      addbasehazard.censor = matrix(0, length(addbase.censor$times), 2)
      addbasehazard.censor[,2] = addbase.censor$times
      addbasehazard.censor[1,1] = addbase.censor$cumhaz[1]*(addbase.censor$times[1] - 0)
      for(t in 2:nrow(addbasehazard.censor)){
    ...
    newdata_A1_censor = data.frame(X.1 = dat$X.1, 
                                   X.2 = dat$X.2, X.3 = dat$X.3, X.4 = dat$X.4, X.5 = dat$X.5,
                                   A = rep(1, nrow(dat)))
    newdata_A0_censor = data.frame(X.1 = dat$X.1, 
                                   X.2 = dat$X.2, X.3 = dat$X.3, X.4 = dat$X.4, X.5 = dat$X.5,
                                   A = rep(0, nrow(dat)))
    
    if(censor == "Cox"){
      coxfit.censor = coxph(Surv(obs.Y, R.censor) ~ X.1 + X.2 + X.3 + X.4 + X.5 + A, data = dat)
      basefit.censor = basehaz(coxfit.censor, centered = FALSE)
      #hazardfit = matrix(0, nrow(basefit), 2)
      #hazardfit[,2] = basefit$time
      #hazardfit[1,1] = basefit$hazard[1]*(basefit$time[1] - 0)
  ...
  for(j in 2:k){
    ...
    if(censor == "Cox"){
      ...
      omega_A1[j,] = 1 - exp(-(baselines.h1)*exp(as.numeric(as.matrix(newdata_A1_censor) %*% as.matrix(summary(coxfit.censor)$coefficients[1:6,1])))) /
        exp(-(baselines.h0)*exp(as.numeric(as.matrix(newdata_A1_censor) %*% as.matrix(summary(coxfit.censor)$coefficients[1:6,1])))) 
      
      omega_A0[j,] =  1 - exp(-(baselines.h1)*exp(as.numeric(as.matrix(newdata_A0_censor) %*% as.matrix(summary(coxfit.censor)$coefficients[1:6,1])))) /
        exp(-(baselines.h0)*exp(as.numeric(as.matrix(newdata_A0_censor) %*% as.matrix(summary(coxfit.censor)$coefficients[1:6,1])))) 
    }else if(censor == "additive"){
      baselines.h = addbasehazard.censor[which.min(abs( addbasehazard.censor[,2] - time.list[j])),1] # baseline
      omega_A1[j,] = baselines.h + as.numeric(as.matrix(newdata_A1_censor) %*% as.matrix(summary(addfit.censor)$coefficients[1:6,1]))
      omega_A0[j,] = baselines.h + as.numeric(as.matrix(newdata_A0_censor) %*% as.matrix(summary(addfit.censor)$coefficients[1:6,1]))
    } 
