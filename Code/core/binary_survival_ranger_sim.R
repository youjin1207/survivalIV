...
binary_survival_ranger_ipw = function(dat, x.trt, x.out, x.miss, x.instrument, 
  w.trt = NULL, w.out = NULL, w.miss = NULL, w.instrument = NULL, time.list, nsplits, misspecify = NULL){
  ...
  for(split in 1:nsplits){
    ...
    for(j in 1:k){
      ...
      Y_Z1_A1 = mean((dat$Y*dat$R*dat$Z*dat$A)[s == split])/mean(omega_Z1_A1*pi_1*delta_1)
      A1_Z1 = mean(dat$A[s == split]*dat$Z[s == split]) / mean(delta_1[s == split])
      
      phi1_1 = mean((dat$Y*dat$R*dat$Z*dat$A/(omega_Z1_A1*pi_1*delta_1))[slong==split])*
        mean((dat$A*dat$Z / (delta_1))[slong==split]) + 
        mean((dat$Y*dat$R*dat$Z*(1-dat$A)/(omega_Z1_A0*(1-pi_1)*delta_1))[slong==split])*
        mean(((1-dat$A)*dat$Z / (delta_1))[slong==split])
      
      phi1_0 =  mean((dat$Y*dat$R*(1-dat$Z)*dat$A/(omega_Z0_A1*pi_0*delta_0))[slong==split])*
        mean((dat$A*(1-dat$Z)/ (delta_0))[slong==split] ) + 
        mean((dat$Y*dat$R*(1-dat$Z)*(1-dat$A)/(omega_Z0_A0*(1-pi_0)*delta_0))[slong==split])*
        mean(((1-dat$A)*(1-dat$Z) / (delta_0))[slong==split])
      ...
      ifvals[s==split, j] = mean(phi1_1 - phi1_0) / 
        mean(phi2_1[slong==split] - phi2_0[slong==split]) 
    }
  }
...
binary_survival_ranger_plugin = function(dat, x.trt, x.out, x.miss, x.instrument, 
  w.trt = NULL, w.out = NULL, w.miss = NULL, w.instrument = NULL, time.list, nsplits, misspecify = NULL){
  ...
  for(split in 1:nsplits){
    ...
    for(j in 2:k){
      ...
      if("censoring" %in% misspecify){
        ...
        missmod = ranger(Y ~., data = cbind(w.out, Y = dat$Y)[slong!=split,])
        omega_Z1_A1[j,] = predict(missmod, data = w.miss_Z1_A1)$predictions
        omega_Z1_A0[j,] = predict(missmod, data = w.miss_Z1_A0)$predictions
        omega_Z0_A1[j,] = predict(missmod, data = w.miss_Z0_A1)$predictions
        omega_Z0_A0[j,] = predict(missmod, data = w.miss_Z0_A0)$predictions
      }else{
        x.miss$I = dat$I;
        x.miss_Z1_A1$I = x.miss_Z1_A0$I = x.miss_Z0_A1$I = x.miss_Z0_A0$I = rep(1, nrow(dat))
        missmod = ranger(Y ~., data = cbind(x.out, Y = dat$Y)[slong!=split,])
        omega_Z1_A1[j,] = predict(missmod, data = x.miss_Z1_A1)$predictions
        omega_Z1_A0[j,] = predict(missmod, data = x.miss_Z1_A0)$predictions
        omega_Z0_A1[j,] = predict(missmod, data = x.miss_Z0_A1)$predictions
    ...
    D_A1 = D_A0 = matrix(0, length(time.list), nrow(dat))
	 ...
	 for(j in 2:k){
       ...
       if("outcome" %in% misspecify){
        ...
        outmod = ranger(Y ~., data = cbind(w.out, Y = dat$Y)[slong!=split,])
        h_A1[j,] = predict(outmod, data = w.out_A1)$predictions
        h_A0[j,] = predict(outmod, data = w.out_A0)$predictions
      }else{
        x.out$I = dat$I;
      ...
      if("censoring" %in% misspecify){
        ...
        missmod = ranger(Y ~., data = cbind(w.out, Y = dat$Y)[slong!=split,])
        omega_A1[j,] = predict(missmod, data = w.miss_A1)$predictions
        omega_A0[j,] = predict(missmod, data = w.miss_A0)$predictions
